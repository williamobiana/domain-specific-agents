...
6. WHEN a transaction description wraps across multiple PDF rows — common for Faster Payments entries with `Reference: …` lines and for currency-conversion annotations — THEN the system SHALL re-join those continuation rows into a single `description` string, separated by a single space. The parser uses word-position analysis (`pdfplumber.extract_words()`): rows are grouped by y-coordinate, a continuation row is identified by having description-column words but no date or amount words, and it is assigned to the nearest transaction row using a midpoint rule on the y-axis. The joining logic SHALL be contained entirely within `parser.py`.
7. WHEN the PDF contains Pot statement pages (pages describing Pot accounts rather than the main account) THEN the system SHALL detect and skip them entirely. No `Transaction` records SHALL be emitted from Pot pages. The account section always precedes Pot pages. Detection is done by checking whether the first non-empty line of the page starts with `"pot statement"` (case-insensitive); once a Pot page is detected, all subsequent pages are skipped.
8. WHEN transaction dates appear in the PDF THEN the system SHALL parse them as `DD/MM/YYYY` (e.g. `01/02/2026`) using `datetime.strptime(date_str, "%d/%m/%Y")`; the parser SHALL NOT rely on the current system date to expand years.
...
12. WHEN the first page of the PDF contains period-level summary totals THEN the system SHALL extract `Total deposits` (preceded by `+£` in pdfplumber output) and `Total outgoings` (preceded by `-£`) as `Decimal` values and store them on the `Statement` object. The statement period is expressed as `DD/MM/YYYY - DD/MM/YYYY` (slash-separated, dash separator).
13. WHEN extracting balance figures THEN the system SHALL extract the closing balance as the last `£X.XX` value appearing before the "Personal Account balance" section marker on the first page. The opening balance is NOT labeled explicitly in Monzo PDFs; it SHALL be derived arithmetically as `closing_balance - total_deposits + total_outgoings` and stored on the `Statement` object.
...
2. WHEN `--rules` is not supplied THEN the system SHALL first look for a rules file at the project-local location `rules/monzo_rules.yaml` (relative to the working directory), then fall back to `~/.config/monzo-expense/rules.yaml`; if neither is present, the system SHALL exit with code 4 and display a usage message listing both paths.
...
The live rules file lives at `rules/monzo_rules.yaml` in the project root (the project-local default discovered by the CLI).

#### Acceptance Criteria

1. WHEN the rules file is loaded THEN it SHALL include a rule mapping `O Okwu-Boms (Faster Payments)` (exact, `direction: in`) to `Main Account Inflow`, and a separate `match_regex: "OMASIRICHI OKWU-BOMS"` rule (`direction: in`) also mapped to `Main Account Inflow` to cover the uppercase variant seen on some transfers.
2. WHEN the rules file is loaded THEN it SHALL include rules for known food-supply counterparties: exact matches for `W M MORRISONS DUMFRIES GBR`, `WM MORRISONS STORE DUMFRIES GBR`, `MORRISONS DUMFRIES - 1 DUMFRIES GBR`, `Lidl GB DUMFRIES GBR`, `TESCO STORES 2388 DUMFRIES GBR`, `POUNDLAND LTD - 2114 DUMFRIES GBR`, and `FARMFOODS 291 DUMFRIES GBR`; a `match_regex: "^MARKS&SPENCER PLC SACA DUMFRIES GBR"` to handle page-boundary continuation; and a `match_regex: "^HOME BARGAINS"` to cover multiple location variants — all mapped to `Food Supplies`.
3. WHEN the rules file is loaded THEN it SHALL include rules for known eating-out counterparties: exact matches for `Eatery 53 Dumfries GBR`, `DGHB CATERING DUMFRIES GBR`, `MARCHBANK BAKERS THORNHILL DG3 GBR`, `La Dolce Vita Dumfries GBR`, `Enish Glasgow Glasgow GBR`, `PPOINT_*McEwans Premie Dumfries GBR`, and `DC7 VENDING LIMITED AYRSHIRE GBR`; and a `match_regex: "^NYX\\*DCVendingLtd"` for vending location variants — all mapped to `Eating Out`.
4. WHEN the rules file is loaded THEN it SHALL include exact-match rules for phone/internet bills: `Lebara Mobile Limited London GBR` and `THREE MOTO GLASGOW GBR`, both mapped to `Bill - Phone & Internet`.
5. WHEN the rules file is loaded THEN it SHALL include rules for known Sundry counterparties: `match_regex: "^RCGP"` and `match_regex: "^GENERAL MEDICAL C"` (descriptions carry a reference suffix); `match_regex: "MEDCOUNCIL/CONSEILMED"` (no `^` anchor — may appear mid-description); `match_regex: "^Amount: CAD"` with `direction: out` (CAD currency-conversion orphan row when the MEDCOUNCIL merchant name wraps a page); exact matches for `RP*My Local Surgery Lt Romsey GBR`, `DUMFRIES HOSPITALS LEA DUMFRIES GBR`, `SAVERS HEALTH & BEAUTY DUMFRIES GBR`, `SUPERDRUG STORES PLC DUMFRIES GBR`, `HOLLAND AND BARRETT DUMFRIES GBR`, `BOOTS 2265 LUTON GBR`, and `POST OFFICE COUNTER DUMFRIES GBR`; `match_regex: "^Ali Mohammad Almasri"` and `match_regex: "^Faizan Akbar"` (housemate/bill-split transfers with reference suffixes); and `match_regex: "^Reference: Ali"` with `direction: out` (standalone reference-only row when the merchant name wrapped to the previous page) — all mapped to `Sundry`.
6. WHEN the rules file is loaded THEN it SHALL include rules for known Holidays & Travel counterparties: `match_regex: "^HOUSTONS MINI COACHES"` (description format varies by location); exact `UBER *TRIP London GBR`; `match_regex: "UBER \\* PENDING London GBR"` with `direction: out` mapped to `Holidays & Travel` AND a second rule for the same regex with `direction: in` mapped to `Unexpected / Refund` (reversal/refund of a pending charge); exact matches for `HARTHILL NORTH SF CONN SHOTTS LANARK GBR`, `ACA KIRKCALDY MG KIRKCALDY GBR`, `VF SERVICES (UK) LTD LONDON GBR`, and `SumUp *McLeans taxi Dumfries GBR`.
7. WHEN the rules file is loaded THEN it SHALL include rules for Car & Gas: `match_regex: "^Adamira Driving School"` (descriptions carry a reference suffix), exact `DVSA SWANSEA GBR`, and exact `HASTINGS DIRECT BEXHILL ON SE GBR` — all mapped to `Car & Gas`.
8. WHEN the rules file is loaded THEN it SHALL include rules for Gifts/Entertainment/Misc: exact matches for `AMAZON.CO.UK LONDON GBR`, `T K MAXX DUMFRIES GBR`, and `BLUE INC - DUMFRIES DUMFRIES GBR`; `match_regex: "Vinted"` with `direction: out` mapped to `Gifts/Entertainment/Misc`; and `match_regex: "Vinted"` with `direction: in` mapped to `Unexpected / Refund` (sale income from Vinted).
9. WHEN the rules file is loaded THEN it SHALL include `match_regex: "^Somtochukwu Nchekwubechukwu Obiana"` with `direction: in` mapped to `Unexpected / Refund` and a separate rule for the same regex with `direction: out` mapped to `Charity / Donations`.
10. WHEN the rules file is loaded THEN it SHALL include `match_regex: "WWW\\.HL\\.CO\\.UK BRISTOL GBR"` with `direction: in` mapped to `Stocks & Shares` (Asset Liquidation) and a separate rule for the same regex with `direction: out` mapped to `Stocks & Shares ISA` (Assets).
11. WHEN the rules file is loaded THEN it SHALL include `match_regex: "^Transfer to Pot"` mapped to `Active Savings` (regex needed because descriptions sometimes include the next transaction's name appended due to page-boundary text joining).
12. WHEN the rules file is loaded THEN it SHALL include rules for `Omasirichi Okwu-Boms` outflows: `match_regex: "^Omasirichi Okwu.Boms \\(Faster Payments\\)"` with `direction: out` mapped to `Charity / Donations` (self-transfers to the main Lloyds account), and `match_regex: "^Reference: Omasirichi"` with `direction: out` mapped to `Charity / Donations` (standalone reference-only rows when the merchant name wrapped to the previous page).
