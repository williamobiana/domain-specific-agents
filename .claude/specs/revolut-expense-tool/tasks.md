...
- [ ] 1. Extend project structure for `revolut_expense` package
...
- [ ] 4.2 Implement Balance summary and period metadata extraction
...
- [ ] 4.3 Implement section detection state machine
...
- [ ] 4.4 Implement description line-joining with continuation patterns and fee absorption
...
- [ ] 4.5 Implement amount column parsing and date parsing
...
- [ ] 4.6 Implement `parse_statement(path: Path) -> Statement`
...
- [ ] 4.7 Create fixtures and write `tests/revolut/test_parser.py`
...
- [ ] 5.3 Write `tests/revolut/test_rules.py` unit tests
...
- [ ] 6.2 Define `ClassifiedTransaction` and `ClassificationResult` dataclasses
...
- [ ] 6.4 Write `tests/revolut/test_classifier.py` unit tests
...
- [ ] 7. Implement `splitter.py` — calendar month grouping
- [ ] 7.1 Define `YearMonth` named tuple
  - Write `class YearMonth(NamedTuple): year: int; month: int`
  - _Requirements: R6.1_

- [ ] 7.2 Implement `split_by_month(result: ClassificationResult) -> dict[YearMonth, ClassificationResult]`
...
- [ ] 7.3 Write `tests/revolut/test_splitter.py` unit tests
...
- [ ] 8. Implement `reconciler.py` — period-level arithmetic verification
- [ ] 8.1 Define `ReconciliationReport` dataclass
...
- [ ] 8.2 Implement `reconcile(result: ClassificationResult, statement: Statement) -> ReconciliationReport`
...
- [ ] 8.3 Write `tests/revolut/test_reconciler.py` unit tests
...
- [ ] 9. Implement `writer.py` — multi-month CSV output
- [ ] 9.1 Implement category total accumulation helper
...
- [ ] 9.2 Implement `write_csvs(by_month, statement, out_dir) -> list[Path]`
...
- [ ] 9.3 Create golden files and write `tests/revolut/test_writer.py`
...
- [ ] 10. Implement `cli.py` — entry point and I/O boundary
- [ ] 10.1 Set up `typer` app and command signature
...
- [ ] 10.2 Wire the full pipeline with error handling
...
- [ ] 10.3 Write `tests/revolut/test_cli.py` integration tests using `typer.testing.CliRunner`
  - Write tests:
...
- [ ] 11. Create live rules file `rules/revolut_rules.yaml`
...
- [ ] 12. Enforce code quality and coverage gates
