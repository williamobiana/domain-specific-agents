...
def load_rules(path: Path) -> list[Rule]:
    ...
    for idx, entry in enumerate(entries):
        ...
        # Reject type field — Monzo rules do not support type-code filtering
        if "type" in entry:
            raise RulesConfigError(
                f"Rule at line {line_number} contains a 'type' field. "
                "Monzo rules do not support type-code filtering; "
        ...
        try:
            ...
            raise RulesConfigError(
                f"Unknown category: {raw_category!r}", line_number=line_number
            )
    ...
    if duplicate_groups:
        violations = [
            f"Lines {', '.join(str(ln) for ln in group)}" for group in duplicate_groups
        ]
