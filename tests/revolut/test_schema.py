"""Tests for revolut_expense/schema.py."""

import pytest

from revolut_expense.schema import (
    SCHEMA_ORDER,
    Category,
    SchemaRow,
    Section,
...
def test_main_account_inflow_in_irregular_inflows_section() -> None:
    for row in SCHEMA_ORDER:
        if row.kind == "line_item" and row.category == Category.MAIN_ACCOUNT_INFLOW:
            assert row.section == Section.IRREGULAR_INFLOWS
